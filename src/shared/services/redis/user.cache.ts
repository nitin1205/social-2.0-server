...
export class UserCache extends BaseCache {
  ...
  public async getRandomUsersFromCache(userId: string, exlcudedUsername: string): Promise<IUserDocument[]> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
      };
      ...
      const exlcudedUserNameIndex: number = findIndex(replies, ['username', exlcudedUsername]);
      replies.splice(exlcudedUserNameIndex, 1);
