...
export class MessageCache extends BaseCache {
  ...
  public async markMessageAsDeleted(senderId: string, receiverId: string, messageId: string, type: string): Promise<IMessageData> {
    try {
      ...
      const { index, receiver, message } = await this.getMessage(senderId, receiverId, messageId);
  ...
  public async updateChatMessage(senderId: string, receiverId: string): Promise<IMessageData> {
    try {
      if(!this.client.isOpen) {
        await this.client.connect();
      };
