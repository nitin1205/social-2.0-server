...
export class GetProfile {
  ...
  public async profileAndPosts(req: Request, res: Response): Promise<void> {
    ...
    const cachedUser: IUserDocument = (await userCache.getUserFromCache(userId)) as IUserDocument;
    const cachedUserPosts: IPostDocument[] = await postCache.getUserPostFromCache('post', parseInt(uId, 10));

    const existingUser: IUserDocument = cachedUser ? cachedUser : await userService.getUserById(userId);
    const userPost: IPostDocument[] = cachedUserPosts.length
      ? cachedUserPosts
      : await postService.getPosts({ username: userName }, 0, 100, { createdAt: -1 });

    res.status(HTTP_STATUS.OK).json({ message: 'Get user profile and posts', user: existingUser, posts: userPost });
  };
  ...
  public async randomUserSuggestions(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'User suggestion', users: randomUsers });
  };
